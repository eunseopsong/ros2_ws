...
car_center_point_x = 317
car_center_point_y = 445
...
class IntegrateInfo():
  ...
  def process(self, data): 
    grad = data.slope
    if grad > 0:
      steering_command = min(7, grad/2)
    else:
      steering_command = max(-7, grad/2)
    ...
    return int(steering_command), actual_left_speed_command, actual_right_speed_command
